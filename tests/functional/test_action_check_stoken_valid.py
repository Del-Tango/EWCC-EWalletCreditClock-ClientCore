...
class TestEwalletClientExecuteActionCheckSTokenValid(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.core.setup_handlers(
            handlers=['action'],
            actions=[
                'RequestClientID', 'RequestSessionToken', 'CheckSTokenValid'
            ]
        )
    ...
    def test_ewcc_set_core_execute_action_check_stoken_valid_functional(self):
        print('\n[ * ]: EWCC Subroutine Execute Action CheckSTokenValid -')
        execute = self.core.execute('CheckSTokenValid')
        print(
            "[ I ]: core.execute('CheckSTokenValid') \n"
            + "[ O ]: " + str(execute) + '\n'
        )
