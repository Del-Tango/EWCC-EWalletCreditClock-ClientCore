...
class TestEwalletClientExecuteActionTransferCredits(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.core.setup_handlers(
            ...
            actions=[
                ...
                'AccountLogin', 'UnlinkAccount', 'TransferCredits'
            ]
        )
        ...
        cls.core.set_values(
            ...
            **{
                'client_id': cls.client_id.get('client_id'),
                'session_token': cls.session_token.get('session_token'),
                'transfer_to': 'ewsc.systemcore@alvearesolutions.ro',
                'credits': 10,
            }
        )
    ...
    def test_ewcc_set_core_execute_action_transfer_credits_functional(self):
        print('\n[ * ]: EWCC Subroutine Execute Action TransferCredits -')
        execute = self.core.execute('TransferCredits')
        print(
            "[ I ]: core.execute('TransferCredits') \n"
            + "[ O ]: " + str(execute) + '\n'
        )
        ...
        self.assertTrue(isinstance(execute.get('transfered_credits'), int))
        self.assertTrue(isinstance(execute.get('transfered_to'), str))
