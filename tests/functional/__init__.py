from . import test_action_request_client_id
from . import test_action_request_session_token

from . import test_action_check_ctoken_valid
from . import test_action_check_ctoken_linked
from . import test_action_check_ctoken_session
from . import test_action_check_ctoken_status
from . import test_action_check_stoken_valid
from . import test_action_check_stoken_linked
from . import test_action_check_stoken_session
from . import test_action_check_stoken_status

from . import test_action_master_create_account
from . import test_action_master_account_login
from . import test_action_master_account_logout
from . import test_action_master_view_account
from . import test_action_master_edit_account
from . import test_action_master_unlink_account
from . import test_action_master_recover_account
from . import test_action_master_inspect_ctokens
from . import test_action_master_inspect_ctoken
from . import test_action_master_inspect_subpool
from . import test_action_master_inspect_subordonate
from . import test_action_master_view_login
from . import test_action_master_view_logout

from . import test_action_acquire_master
from . import test_action_stoken_keep_alive
from . import test_action_ctoken_keep_alive
from . import test_action_issue_report
from . import test_action_release_master

from . import test_action_create_account
from . import test_action_account_login
from . import test_action_account_logout
from . import test_action_supply_credits
from . import test_action_pay_credits
from . import test_action_convert_credits2clock
from . import test_action_convert_clock2credits
from . import test_action_add_contact_record
from . import test_action_transfer_credits
from . import test_action_edit_account
from . import test_action_recover_account

from . import test_action_clock_timer_start
from . import test_action_clock_timer_pause
from . import test_action_clock_timer_resume
from . import test_action_clock_timer_stop

from . import test_action_view_contact_list
from . import test_action_view_contact_record
from . import test_action_view_transfer_sheet
from . import test_action_view_transfer_record
from . import test_action_view_time_sheet
from . import test_action_view_time_record
from . import test_action_view_conversion_sheet
from . import test_action_view_conversion_record
from . import test_action_view_account
from . import test_action_view_credit_ewallet
from . import test_action_view_credit_clock
from . import test_action_view_invoice_sheet
from . import test_action_view_invoice_record
from . import test_action_view_login_records
from . import test_action_view_logout_records

from . import test_action_create_credit_ewallet
from . import test_action_create_credit_clock
from . import test_action_create_transfer_sheet
from . import test_action_create_invoice_sheet
from . import test_action_create_conversion_sheet
from . import test_action_create_time_sheet
from . import test_action_create_contact_list

from . import test_action_switch_session_user
from . import test_action_switch_credit_ewallet
from . import test_action_switch_credit_clock
from . import test_action_switch_transfer_sheet
from . import test_action_switch_invoice_sheet
from . import test_action_switch_conversion_sheet
from . import test_action_switch_time_sheet
from . import test_action_switch_contact_list

from . import test_action_unlink_account
from . import test_action_unlink_contact_list
from . import test_action_unlink_contact_record
from . import test_action_unlink_conversion_sheet
from . import test_action_unlink_conversion_record
from . import test_action_unlink_transfer_sheet
from . import test_action_unlink_transfer_record
from . import test_action_unlink_invoice_sheet
from . import test_action_unlink_invoice_record
from . import test_action_unlink_time_sheet
from . import test_action_unlink_time_record
from . import test_action_unlink_credit_ewallet
from . import test_action_unlink_credit_clock
