...
class TestEwalletClientExecuteActionCheckCTokenSession(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.core.setup_handlers(
            handlers=['action'],
            actions=[
                'RequestClientID', 'RequestSessionToken', 'CheckCTokenSession'
            ]
        )
        ...
        print('[...] Subroutine Set ResourceInstruction')
        cls.core.set_values(
            'CheckCTokenSession', **{'client_id': cls.client_id.get('client_id')}
        )
    ...
    def test_ewcc_set_core_execute_action_check_ctoken_session_functional(self):
        print('\n[ * ]: EWCC Subroutine Execute Action CheckCTokenSession -')
        execute = self.core.execute('CheckCTokenSession')
        print(
            "[ I ]: core.execute('CheckCTokenSession') \n"
            + "[ O ]: " + str(execute) + '\n'
        )
