...
class TestEwalletClientExecuteActionStartClockTimer(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        convert_credit2clock = cls.core.execute('ConvertCreditsToClock')
    ...
    def test_ewcc_set_core_execute_action_start_clock_timer_functional(self):
        print('\n[ * ]: EWCC Subroutine Execute Action StartClockTimer -')
        execute = self.core.execute('StartClockTimer')
        print(
            "[ I ]: core.execute('StartClockTimer') \n"
            + "[ O ]: " + str(execute) + '\n'
        )
