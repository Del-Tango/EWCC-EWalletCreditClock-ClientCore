    ...
    def setUpClass(cls):
        ...
        setup_handlers = cls.core.setup_handlers(
            ...
            actions=[
                ...
                'ViewCreditEWallet', 'CreateMaster', 'AcquireMaster',
        ...
        print('[...]: Subroutine Execute AccountLogin')
        view_ewallet = cls.response = cls.core.execute('ViewCreditEWallet')

        ewallet_data = cls.response.get('ewallet_data')
        cls.current_clock = int() if not ewallet_data else \
            cls.response['ewallet_data']['clock']
        cls.new_clock = int() if not ewallet_data else int(
            list(
