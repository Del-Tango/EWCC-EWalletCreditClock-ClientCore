...
class TestEwalletClientExecuteActionMasterInspectCTokens(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        setup_handlers = cls.core.setup_handlers(
            ...
            actions=[
                ...
                'InspectCTokens', 'MasterUnlinkAccount'
            ]
        )
    ...
    def test_ewcc_set_core_execute_action_master_inspect_ctokens_functional(self):
        print('\n[ * ]: EWCC Subroutine Execute Action InspectCTokens -')
        execute = self.core.execute('InspectCTokens')
        print(
            "[ I ]: core.execute('InspectCTokens') \n"
            + "[ O ]: " + str(execute) + '\n'
        )
        ...
        self.assertTrue(isinstance(execute.get('ctokens'), list))
        return execute
