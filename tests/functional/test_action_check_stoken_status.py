...
class TestEwalletClientExecuteActionCheckSTokenStatus(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.core.setup_handlers(
            handlers=['action'],
            actions=[
                'RequestClientID', 'RequestSessionToken', 'CheckSTokenStatus'
            ]
        )
    ...
    def test_ewcc_set_core_execute_action_check_stoken_status_functional(self):
        print('\n[ * ]: EWCC Subroutine Execute Action CheckSTokenStatus -')
        execute = self.core.execute('CheckSTokenStatus')
        print(
            "[ I ]: core.execute('CheckSTokenStatus') \n"
            + "[ O ]: " + str(execute) + '\n'
        )
