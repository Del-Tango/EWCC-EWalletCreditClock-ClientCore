...
class TestEwalletClientExecuteActionViewCreditClock(unittest.TestCase):
...
#   @pysnooper.snoop()
    def setUpClass(cls):
        ...
        setup_handlers = cls.core.setup_handlers(
            ...
            actions=[
                ...
                'AccountLogin', 'UnlinkAccount', 'ViewCreditClock',
                'CreateMaster', 'MasterAccountLogin', 'AcquireMaster',
                'MasterUnlinkAccount',
            ]
        )
    ...
    def test_ewcc_set_core_execute_action_view_credit_clock_functional(self):
        print('\n[ * ]: EWCC Subroutine Execute Action ViewCreditClock -')
        execute = self.core.execute('ViewCreditClock')
        print(
            "[ I ]: core.execute('ViewCreditClock') \n"
            + "[ O ]: " + str(execute) + '\n'
        )
