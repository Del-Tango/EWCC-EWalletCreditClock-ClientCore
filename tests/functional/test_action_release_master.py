...
class TestEwalletClientExecuteActionReleaseMasterAccount(unittest.TestCase):
...
#   @pysnooper.snoop('logs/ewcc.log')
    def setUpClass(cls):
        ...
        setup_handlers = cls.core.setup_handlers(
            ...
            actions=[
                ...
                'ReleaseMaster', 'AcquireMaster', 'MasterAccountLogin',
                'MasterUnlinkAccount'
            ]
        )
    ...
    def test_ewcc_set_core_execute_action_release_master_account_functional(self):
        print('\n[ * ]: EWCC Subroutine Execute Action ReleaseMaster -')
        execute = self.core.execute('ReleaseMaster')
        print(
            "[ I ]: core.execute('ReleaseMaster') \n"
            + "[ O ]: " + str(execute) + '\n'
        )
        ...
        self.assertTrue(isinstance(execute.get('released'), str))
