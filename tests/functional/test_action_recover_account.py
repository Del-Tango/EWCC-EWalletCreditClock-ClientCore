...
#   @pysnooper.snoop()
    ...
    def test_ewcc_set_core_execute_action_recover_account_functional(self):
        print('\n[ * ]: EWCC Subroutine Execute Action RecoverAccount -')
        execute = self.core.execute('RecoverAccount')
        print(
            "[ I ]: core.execute('RecoverAccount') \n"
            + "[ O ]: " + str(execute) + '\n'
        )
